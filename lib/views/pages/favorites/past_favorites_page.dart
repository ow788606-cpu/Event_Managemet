...
class PastFavoritesPage extends StatefulWidget {
  const PastFavoritesPage({super.key});

  @override
  State<PastFavoritesPage> createState() => _PastFavoritesPageState();
}

class _PastFavoritesPageState extends State<PastFavoritesPage> {
  List<dynamic> events = [];
  bool isLoading = true;
  ...
  Future<void> _loadEvents() async {
    try {
      ...
      setState(() {
        events = data.where((e) => _isPast(e['start_date']?.toString() ?? '')).toList();
        isLoading = false;
      });
    } catch (e) {
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('No past events', style: TextStyle(fontSize: width * 0.045, color: Colors.grey, fontFamily: 'Inter')),
                ],
              ),
            )
          : ListView.builder(
